...
void load_gt_google_qa(const std::string& filename, std::vector<std::vector<uint>>& graph, size_t& dataset_size,
                       size_t& k);

/* evaluation */
double measure_neighbors_accuracy(const std::vector<uint>& gt_neighbors,
                                  const std::vector<std::pair<float, uint>>& neighbors, int k);
...
double measure_graph_quality(const std::vector<std::vector<uint>>& gt_graph,
    ...
    for (uint x = 0; x < dataset_size; x++) {
        ...
        for (uint i = 0; i < k; i++) {
            ...
            if (std::find(gt_neighbors.begin() + 1, gt_neighbors.begin() + k + 1, est_node) !=
                gt_neighbors.begin() + k + 1) {
                total_num_correct++;
            }
        }
    }
...
void load_dataset_google_qa(const std::string& filename, float*& data_pointer, size_t& dataset_size,
                            size_t& dimension) {
