...
def compress_dataset(data, num_bits):
    ...
    l_arr = np.zeros((N), dtype=np.float32)
    r_arr = np.zeros((N), dtype=np.float32)
    for i in range(N):
        compressed_dataset[i,:], _, l_arr[i], r_arr[i] = quantize_vector(data[i,:], num_bits)
...
def uncompress_dataset(compressed_dataset_object):
    ...
    for i in range(N):
        l = compressed_dataset_object['l_arr'][i]
        r = compressed_dataset_object['r_arr'][i]
